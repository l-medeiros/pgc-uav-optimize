...
def build_visit_order(
    ...
    while unvisited:
        nxt = min(
            unvisited,
            key=lambda j: (nodes_map.distances[(current, j)], j),
        )

        move_cost = energy_cost[(current, nxt)]
        return_cost = energy_cost[(nxt, base_id)]
...
def main():
    ...
    parser.add_argument("--aoi-state",     default="setup/aoi_state_nn.csv")
    parser.add_argument("--aoi-history",   default="setup/aoi_history_nn.csv")
    parser.add_argument("--round-summary", default="setup/round_summary_nn.csv")
    ...
    visit_order = build_visit_order(nodes_map, base.id, sensor_ids, energy_cost)
