...
\begin{document}
    ...
    \begin{resumo}
        A coleta de dados em redes de sensores da Internet das Coisas (IoT) por meio de veículos aéreos não tripulados (VANTs) exige o planejamento cuidadoso da trajetória de voo, de modo a equilibrar a qualidade da informação coletada e o consumo energético do veículo. Este trabalho propõe uma formulação de Programação Linear Inteira Mista (MILP) para o problema de otimização do plano de voo de um VANT dedicado à coleta de dados de sensores distribuídos em uma área geográfica. O modelo discretiza o horizonte da missão em intervalos de tempo (\textit{slots}) e descreve o deslocamento do veículo entre os sensores e a base, incorporando um modelo de potência de asa rotativa para estimar o custo energético de cada deslocamento e dos períodos de pairar (\textit{hovering}). A idade da informação (AoI, do inglês \textit{Age of Information}) é utilizada como medida da relevância temporal dos dados, e a missão é otimizada de forma multiobjetivo lexicográfica: maximiza-se primeiro a AoI efetivamente coletada e, em seguida, minimiza-se a energia consumida. A formulação foi implementada no resolvedor Gurobi e avaliada em 36 cenários, variando a quantidade de sensores e o tamanho do mapa, com parâmetros físicos do DJI Matrice 300 RTK e do Parrot Anafi USA, e comparada a heurísticas construtivas de referência (\textit{Nearest Neighbor}, AoI-Greedy e Score-Greedy). Os resultados indicam que o modelo exato encontra soluções de boa qualidade nas escalas avaliadas e que a restrição de visita única a cada sensor constitui uma escolha eficiente, obtendo uma qualidade de AoI equivalente àquela obtida por uma variante com revisitas, porém com consumo energético significativamente menor.

        \vspace{\onelineskip}
        \noindent\textbf{Palavras-chave}: VANT, Internet das Coisas, idade da informação, otimização, programação linear inteira mista, eficiência energética.
    \end{resumo}
...
O modelo de comunicação adotado é simplificado: assume-se que a transmissão ocorre com sucesso sempre que o VANT permanece pairando diretamente sobre o sensor durante um slot completo de duração $\Delta t$. Não são modelados aspectos de camada de enlace como modulação, interferência ou protocolo MAC. A coleta é representada por uma variável binária que indica se o drone está posicionado sobre o sensor no instante correspondente (ver Equação~\eqref{eq:coleta}).


    \section{Parâmetros e Variáveis}

O modelo representa o voo do VANT como uma sequência de estados discretos no tempo, divididos em slots de duração $\Delta t$. O espaço de movimentação é modelado como um grafo completo sobre o conjunto de nós $N = S \cup \{b\}$ (sensores e base), com coordenadas bidimensionais $(x, y)$. O custo energético de cada transição é determinado pelo modelo de potência de asa rotativa em função da velocidade de voo. As variáveis de decisão binárias controlam a posição e os deslocamentos do VANT ao longo do horizonte temporal. Variáveis contínuas registram a energia acumulada e a idade da informação (AoI) de cada sensor a cada slot. A solução ótima define uma sequência de posições do VANT que maximiza a informação coletada respeitando a restrição de capacidade de bateria $E^{\max}$. As Tabelas~\ref{tab:parametros} e~\ref{tab:variaveis} listam, respectivamente, os parâmetros e as variáveis de decisão do modelo.
    ...
    \begin{table}[htbp]
      ...
      \begin{tabular}{>{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}p{12.5cm}}
        ...
        $A = N \times N$ & Conjunto de arestas dirigidas (inclui laço próprio $(i,i)$) \\
        \midrule
        ...
        $T_f$ & Horizonte temporal: número máximo de slots disponíveis para a missão \\
        $E^{\max}$ & Capacidade máxima de energia da bateria do VANT \\
        ...
        \midrule
        \multicolumn{2}{l}{\textit{Parâmetros do Modelo de Potência do VANT}} \\
        \midrule
        ...
        $v_0$ & Velocidade induzida em voo pairado \\
        $\rho$ & Densidade do ar \\
    ...
    é planejar a trajetória de um ou mais VANTs para coleta eficiente de dados em sensores IoT, respeitando restrições de mobilidade, energia e requisitos de comunicação. O problema pode ser formulado pelas equações descritas nas subseções a seguir.

    \subsection{Definição do Grafo de Movimento}

    O conjunto de posições possíveis para o VANT é dado por    
    $N = S \cup \{b\}$, onde $b$ representa a base.  
    Diferentemente de modelos que usam grafos esparsos, aqui adotamos $A = N \times N$, um grafo dirigido completo com laços próprios (\emph{self-loops}).  
    Isso significa que:
    
    \begin{itemize}
        \item o VANT pode transitar de qualquer nó para qualquer outro em um único slot (pressuposto do grafo completo, embora na prática transições longas sejam desincentivadas pelo custo energético $c_{ij}$),
        \item o caso $(i,i)$ modela o \textbf{voo pairado}, isto é, o VANT permanece parado,
        \item a dinâmica de movimento é totalmente determinada pelas variáveis $x_{ijt}$. 
    \end{itemize}
    
    O uso de um grafo completo fornece maior generalidade, permitindo representar tanto deslocamentos quanto posicionamento estático. A Figura~\ref{fig:grafo-movimento} ilustra esse grafo para um exemplo pequeno, com a base e três sensores.
    ...
    \begin{figure}[htbp]
      ...
      \begin{tikzpicture}[
        ...
        place/.style={circle, draw, thick, minimum size=10mm, inner sep=1pt},
        sensor/.style={place, fill=blue!8},
        base/.style={place, fill=orange!25},
      ]
        \node[base] (b) at (0,0) {$b$};
        \node[sensor] (s1) at (3,1.6) {$s_1$};
        \node[sensor] (s2) at (3,-1.6) {$s_2$};
        \node[sensor] (s3) at (6,0) {$s_3$};
        ...
        \draw[thick] (s3) to[out=-40, in=40, looseness=9]
          node[right=1pt]{\footnotesize pairar/coleta ($x_{jjt}$)} (s3);
      \end{tikzpicture}
      \caption{Exemplo do grafo de movimento $N = S \cup \{b\}$ com a base $b$ e três sensores. As arestas representam os deslocamentos possíveis do VANT entre os pares de nós em um \textit{slot}. O laço próprio $(j,j)$ representa o voo pairado do VANT sobre o nó, associado à coleta quando $v_{j,t} = x_{jjt} = 1$.}
      \label{fig:grafo-movimento}
    \end{figure}

    \subsubsection{Interpretação de $x_{ijt}$}

    A variável binária $x_{ijt}$ indica que o VANT se encontra no nó $i$ no início do slot $t$ e estará no nó $j$ no início do slot $t+1$.
    Assim:

    \[
    x_{ijt} =
    \begin{cases}
    1, & \text{se o VANT realiza a transição } i \to j \text{ no tempo } t, \\[4pt]
    ...
    x_{iit} = 1
    \]
    implica que o VANT permanece parado (pairando), o que também é utilizado para caracterizar coletas via $v_{j,t} = x_{jjt}$.

    \subsection{Modelo de Energia e Potência do VANT}

    O modelo de consumo energético utilizado é baseado no modelo de potência para VANTs de asa rotativa descrito por \cite{zeng2019rotarywing}, que considera três componentes: (i) potência de perfil das pás, (ii) potência induzida e (iii) potência de arrasto parasita.

    A potência consumida pelo VANT quando se desloca com velocidade $V$ é dada pela Equação~\eqref{eq:potencia}:
    ...
    onde $P_0$, $\Pi$, $U_\text{tip}$, $v_0$, $\rho$ e $C_{DS}$ são parâmetros físicos do VANT.
    ...
    O caso especial de voo pairado corresponde à aresta $(i,i)$, onde $V_{ii}=0$ e portanto:
    ...
    (i) a maximização da quantidade de informação relevante coletada pelo VANT, medida pela idade da informação (AoI), e
    ...
    \subsubsection{Maximização da Informação Coletada (AoI)}
    ...
    Essa função incentiva visitas a sensores que tenham dados acumulados mais antigos, priorizando trajetórias que maximizem o impacto informacional da missão.
    
    \subsubsection{Minimização do Consumo Energético}
    
    A energia acumulada até o final do voo é representada pela variável $E_{T_f}$, que sintetiza todo o gasto resultante dos deslocamentos e períodos de voo pairado. Assim, o custo energético total da missão é expresso por:
    ...
    \subsubsection{Função Objetivo Multicritério}
    ...
    As Restrições~\eqref{eq:mov-base}--\eqref{eq:mov-fluxo} modelam a dinâmica de movimento do VANT ao longo dos slots de tempo. A Restrição~\eqref{eq:mov-base} fixa a base $b$ como ponto de partida e de chegada da missão, garantindo que o drone inicie o voo na base e retorne a ela no final do horizonte $T_f$. A Restrição~\eqref{eq:mov-unicidade} impõe que, em cada slot $t$, o VANT esteja exatamente em um único nó $n \in N$, isto é, o drone não pode estar em dois locais ao mesmo tempo nem ``desaparecer'' do grafo. Já a Restrição~\eqref{eq:mov-fluxo} garante a consistência do fluxo entre nós sucessivos: para cada slot $t$, o número de arestas saindo de um nó $i$ é igual à indicação de que o drone está em $i$ no início do slot, e o número de arestas chegando em um nó $j$ é igual à indicação de que o drone estará em $j$ no início do slot seguinte. Em conjunto, essas restrições asseguram que a trajetória do VANT é uma caminhada viável no grafo $N$ ao longo do horizonte de tempo.

    
    \subsection{Restrições de Comunicação}

    No modelo, uma coleta de dados ocorre somente quando o VANT permanece pairando sobre o sensor durante todo o slot.  
    
    Esse comportamento é modelado por meio de um laço próprio na aresta $(j,j)$:
    ...
    Se o VANT estiver se deslocando entre quaisquer nós distintos $(i \ne j)$, então $x_{j j t} = 0$ e, portanto, nenhuma coleta ocorre.
    ...
    As restrições acima refletem a dinâmica de energia implementada no código, onde $E_t$ é a energia total acumulada e $c_{ij}$ segue a modelagem de potência do VANT.

    \subsection{Restrições de idade da informação (AoI)}
    ...
    \textbf{Caso $v_{j,t} = 0$.}
    Substituindo $v_{j,t}=0$ nas Equações~\eqref{eq:lin-a}--\eqref{eq:lin-c}, obtemos:
    \[
    ...
    \textbf{Caso $v_{j,t} = 1$.}
    Substituindo $v_{j,t}=1$ nas Equações~\eqref{eq:lin-a}--\eqref{eq:lin-c}, obtemos:
    \[
        w_{j,t} \leq A_{j,t}, \qquad
    ...
    \input{metodologia}
    % \input{cronograma} % removido: cronograma não integra a versão final
