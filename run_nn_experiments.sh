...
HEURISTIC="$(dirname "$0")/heuristic_nn.py"
RUNS=30
...
echo "=============================="
echo "UAV Experiments — Nearest Neighbor"
...
for n in "${SENSOR_COUNTS[@]}"; do
    for L in "${MAP_SIZES[@]}"; do
        ...
        RESULTADOS_DIR="$BASE_DIR/$n/resultados_nn"
        AOI_STATE="$BASE_DIR/$n/aoi_state_nn_${L}x${L}.tmp.csv"
