...
A principal técnica para resolver problemas de PLI e MILP de forma exata é o \emph{branch-and-bound} \cite{land1960branchbound, arenales2007pesquisa}. O método parte da \emph{relaxação linear} do problema, na qual se ignora temporariamente a restrição de integralidade, resolvendo-se o PL resultante, cuja solução fornece um limitante para o valor ótimo. Caso a solução relaxada já satisfaça a integralidade, ela é ótima; caso contrário, escolhe-se uma variável com valor fracionário e o problema é dividido (\emph{branching}) em subproblemas que restringem essa variável a faixas inteiras disjuntas. Os limitantes obtidos em cada nó permitem podar (\emph{bounding}) ramos que comprovadamente não contêm a solução ótima, evitando a enumeração completa. Esses métodos exatos, em conjunto com técnicas de planos de corte, são implementados em resolvedores de otimização de uso amplo, como o Gurobi, adotado neste trabalho, capazes de resolver de forma exata instâncias de porte considerável.
