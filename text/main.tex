...
O modelo representa o voo do VANT como uma sequência de estados discretos no tempo, divididos em slots de duração $\Delta t$. O espaço de movimentação é modelado como um grafo completo sobre o conjunto de nós $N = S \cup \{b\}$ (sensores e base), com coordenadas bidimensionais $(x, y)$. O custo energético de cada transição é determinado pelo modelo de potência de asa rotativa em função da velocidade de voo. As variáveis de decisão binárias controlam a posição e os deslocamentos do VANT ao longo do horizonte temporal. Variáveis contínuas registram a energia acumulada e a \textit{Age of Information} (AoI) de cada sensor a cada slot. A solução ótima define uma sequência de posições do VANT que maximiza a informação coletada respeitando a restrição de capacidade de bateria $E^{\max}$. As Tabelas~\ref{tab:parametros} e~\ref{tab:variaveis} listam, respectivamente, os parâmetros e as variáveis de decisão do modelo.
    ...
    \begin{itemize}
        ...
        \item o caso $(i,i)$ modela \textbf{hover}, isto é, o VANT permanece parado,
    ...
    \begin{figure}[htbp]
      ...
      \begin{tikzpicture}[
        ...
        \draw[->, thick] (s3) to[out=-40, in=40, looseness=9]
          node[right=1pt]{\footnotesize \textit{hover}/coleta ($x_{jjt}$)} (s3);
      \end{tikzpicture}
      \caption{Exemplo do grafo de movimento $N = S \cup \{b\}$ com a base $b$ e três sensores. As arestas dirigidas conectam todos os pares de nós (transições $i \to j$ realizadas em um \textit{slot}). O \textit{self-loop} $(j,j)$ representa o \textit{hover} do VANT sobre o nó, associado à coleta quando $v_{j,t} = x_{jjt} = 1$.}
    ...
    x_{iit} = 1
    \]
    implica que o VANT permanece parado (\emph{hover}), o que também é utilizado para caracterizar coletas via $v_{j,t} = x_{jjt}$.
    ...
    (i) a maximização da quantidade de informação relevante coletada pelo VANT, medida pela Age of Information (AoI), e 
