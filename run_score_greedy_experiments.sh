...
HEURISTIC="$(dirname "$0")/heuristic_score_greedy.py"
RUNS=30
...
echo "=============================="
echo "UAV Experiments — Score-Greedy (AoI/dist)"
...
for n in "${SENSOR_COUNTS[@]}"; do
    for L in "${MAP_SIZES[@]}"; do
        ...
        RESULTADOS_DIR="$BASE_DIR/$n/resultados_score_greedy"
        AOI_STATE="$BASE_DIR/$n/aoi_state_score_greedy_${L}x${L}.tmp.csv"
