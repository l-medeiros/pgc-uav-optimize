...
% Grafo de exemplo para ilustrar as heurísticas (base + 4 sensores)
\tikzset{
  hbase/.style={circle, draw, thick, fill=orange!25, minimum size=7mm, inner sep=0pt, font=\scriptsize},
  hsensor/.style={circle, draw, thick, fill=blue!10, minimum size=7mm, inner sep=0pt, font=\scriptsize},
}
\newcommand{\heurgraph}{%
  \node[hbase] (b) at (0,0) {$b$};
  \node[hsensor] (s1) at (1.7,1.3) {$s_1$};
  \node[hsensor] (s2) at (3.4,0.8) {$s_2$};
  \node[hsensor] (s3) at (1.3,-1.5) {$s_3$};
  \node[hsensor] (s4) at (3.8,-1.3) {$s_4$};
  \draw[gray!55] (b)--(s1) (b)--(s2) (b)--(s3) (b)--(s4);
}
...
\begin{frame}{Do cenário real ao problema}
  \begin{columns}[c]
    \begin{column}{0.6\textwidth}
      \small
      \begin{itemize}
        \item Sensores distribuídos, sem infraestrutura fixa, e um VANT como coletor móvel.
...
\begin{frame}{O cenário considerado}
  \begin{itemize}
    ...
    \item Dois objetivos conflitantes:
      ...
      \end{itemize}
    \item O tempo é discretizado em intervalos iguais, os \emph{slots} de duração $\Delta t$.
    ...
    \item Um único VANT, permitindo uma formulação exata.
  \end{itemize}
\end{frame}
...
\begin{frame}{Trabalhos relacionados}
  ...
  \begin{tabular}{p{5.3cm} c c c c c}
    ...
    \midrule
    UAV-Assisted Data Collection for IoT           & 2022 & Múltiplos & Revisão   & Não & Não \\
    A Survey of UAV-based Data Collection          & 2023 & Múltiplos & Revisão   & Não & Não \\
    UAVs Assisted Data Collection in WSN           & 2018 & Múltiplos & Analítico & Não & Não \\
    Learning-Based Data Collection by Vehicles     & 2018 & Múltiplos & DRL       & Sim & Não \\
    Key Issues in UAV Data Collection in IoT       & 2020 & Múltiplos & Revisão   & Não & Não \\
    \midrule
  ...
  \end{center}
  \vspace{0.5em}
  Lacuna: nenhum trabalho anterior \textbf{otimiza AoI e energia conjuntamente}. Este é o único, via formulação exata (MILP, Programa Linear Inteiro Misto) com parâmetros físicos reais de VANTs.
\end{frame}

\section{Formulação}

\begin{frame}{Age of Information e energia}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        \item Modelo de potência de asa rotativa.
...
\begin{frame}{O modelo: Programa Linear Inteiro Misto (MILP)}
  Formulamos o plano de voo como um \textbf{MILP} (\emph{Mixed-Integer Linear Programming}):
  \begin{itemize}
    \item Função objetivo e restrições \textbf{lineares}.
    \item Variáveis \textbf{binárias} (decisões: mover, visitar) e \textbf{contínuas} (energia, AoI).
    \item Resolvido de forma \textbf{exata} (solução ótima garantida) pelo Gurobi, via \emph{branch-and-bound}.
  \end{itemize}
  \vspace{0.4em}
  Serve de \textbf{referência ótima} para avaliar as heurísticas, ao custo de crescer rapidamente com o tamanho da instância.
\end{frame}
...
\begin{frame}{Heurísticas de referência: Nearest Neighbor (NN)}
  \begin{columns}[c]
    \begin{column}{0.55\textwidth}
      \centering
      \begin{tikzpicture}
        \heurgraph
        \node[font=\tiny] at (0.95,0.75) {$d\,2{,}0$};
        \node[font=\tiny] at (1.75,0.35) {$d\,3{,}3$};
        \node[font=\tiny] at (0.6,-0.85) {$d\,1{,}8$};
        \node[font=\tiny] at (2.0,-0.75) {$d\,3{,}8$};
        \draw[green!50!black, very thick] (b)--(s3);
        \node[hsensor, fill=green!35] at (1.3,-1.5) {$s_3$};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.45\textwidth}
      \small
      Visita sempre o sensor viável \textbf{mais próximo} da posição atual.

      \vspace{0.6em}
      Do \emph{base} $b$, o mais próximo é $s_3$ ($d\,1{,}8$).
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Heurísticas de referência: AoI-Greedy}
  \begin{columns}[c]
    \begin{column}{0.55\textwidth}
      \centering
      \begin{tikzpicture}
        \heurgraph
        \node[font=\tiny] at (1.7,1.85) {AoI $8$};
        \node[font=\tiny] at (3.4,1.35) {AoI $9$};
        \node[font=\tiny] at (1.3,-2.05) {AoI $4$};
        \node[font=\tiny] at (3.8,-1.85) {AoI $14$};
        \draw[green!50!black, very thick] (b)--(s4);
        \node[hsensor, fill=green!35] at (3.8,-1.3) {$s_4$};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.45\textwidth}
      \small
      Visita o sensor de \textbf{maior AoI} (prioriza os dados mais antigos).

      \vspace{0.6em}
      Escolhe $s_4$ (AoI $14$), mesmo estando mais longe.
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Heurísticas de referência: Score-Greedy}
  \begin{columns}[c]
    \begin{column}{0.55\textwidth}
      \centering
      \begin{tikzpicture}
        \heurgraph
        \node[font=\tiny] at (1.7,1.85) {$4{,}0$};
        \node[font=\tiny] at (3.4,1.35) {$2{,}7$};
        \node[font=\tiny] at (1.3,-2.05) {$2{,}2$};
        \node[font=\tiny] at (3.8,-1.85) {$3{,}7$};
        \draw[green!50!black, very thick] (b)--(s1);
        \node[hsensor, fill=green!35] at (1.7,1.3) {$s_1$};
      \end{tikzpicture}
    \end{column}
    \begin{column}{0.45\textwidth}
      \small
      Visita o de maior razão \textbf{AoI\,/\,distância}: equilibra urgência e custo.

      \vspace{0.6em}
      Escolhe $s_1$ (razão $4{,}0$), um bom meio-termo.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Conclusões e trabalhos futuros}
  ...
  \begin{itemize}
    ...
    \item Modelos de comunicação considerando a altura do drone.
  \end{itemize}
\end{frame}
