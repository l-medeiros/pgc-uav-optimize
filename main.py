...
def handle_solution(
    ...
    if not has_solution:
        ...
        save_aoi_state(aoi_after, AOI_STATE_PATH)
        r = next_round_index(AOI_HISTORY_PATH)
        append_aoi_history(AOI_HISTORY_PATH, r, aoi_before, aoi_after, visited)
        append_round_summary(
            path=ROUND_SUMMARY_PATH,
            round_idx=r,
            energy_final=0.0,
            collected_aoi=0.0,
...
def main():
    ...
    aoi_before = load_aoi_state(sensor_ids, AOI_STATE_PATH)
