...
HEURISTIC="$(dirname "$0")/heuristic_aoi_greedy.py"
RUNS=30
...
echo "=============================="
echo "UAV Experiments — AoI-Greedy"
...
for n in "${SENSOR_COUNTS[@]}"; do
    for L in "${MAP_SIZES[@]}"; do
        ...
        RESULTADOS_DIR="$BASE_DIR/$n/resultados_aoi_greedy"
        AOI_STATE="$BASE_DIR/$n/aoi_state_aoi_greedy_${L}x${L}.tmp.csv"
