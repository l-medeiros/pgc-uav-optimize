...
O problema de planejamento de voo abordado neste trabalho é formulado como um MILP: variáveis binárias descrevem a posição do UAV, os deslocamentos e as coletas, enquanto variáveis contínuas registram a energia acumulada e a \emph{Age of Information} dos sensores. Termos não lineares, como o produto entre a AoI e a decisão de coleta, são convertidos em restrições lineares por meio de técnicas de linearização (Capítulo~\ref{sec:modelagem}, Equações~\eqref{eq:lin-a}--\eqref{eq:lin-c}), preservando a estrutura linear necessária à resolução exata pelo Gurobi.
...
Os modelos adotados neste trabalho utilizam uma representação de \emph{tempo discreto}: o horizonte de uma missão é particionado em $T_f$ intervalos de igual duração $\Delta t$, denominados \emph{slots}. Em cada slot, o estado do sistema (posição do UAV, energia acumulada e idade da informação dos sensores) é considerado constante, e as decisões de movimento e coleta são tomadas slot a slot. Essa discretização transforma a evolução contínua da missão em uma sequência finita de estados, viabilizando a formulação por programação linear inteira. O conceito de slot, introduzido aqui, é utilizado ao longo de toda a fundamentação e da modelagem.
...
\section{Modelo de energia de UAV de asa rotativa}
...
Não são modelados aspectos de camada de enlace como taxa de dados, modulação, interferência entre sensores ou protocolo MAC. A coleta é representada pela variável binária $v_{j,t} \in \{0,1\}$, que indica se o UAV permanece estacionário sobre o sensor $j$ durante o slot $t$ (Equação~\eqref{eq:coleta}, Capítulo~\ref{sec:modelagem}).

Esta simplificação é adequada por três motivos:

\begin{enumerate}
    \item \textbf{Distância mínima:} ao pairar sobre o sensor, o UAV minimiza a distância de enlace, garantindo SNR suficiente para tecnologias de curto alcance como IEEE 802.15.4, Zigbee ou LoRa, além de linha de visada.
