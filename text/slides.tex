...
\begin{frame}{O que são VANTs?}
  \begin{columns}[c]
    \begin{column}{0.58\textwidth}
      \small
      \begin{itemize}
        \item Veículos aéreos não tripulados (VANTs), ou drones: aeronaves sem piloto a bordo, controladas remotamente ou de forma autônoma.
        \item Dois tipos principais de propulsão:
          ...
          \end{itemize}
        \item O \emph{hover} é essencial para coletar dados parado sobre um sensor.
...
\begin{frame}{O que é IoT?}
  \begin{itemize}
    \item \textbf{Internet das Coisas (IoT):} objetos físicos (sensores, atuadores, dispositivos embarcados) conectados, que coletam e transmitem dados do ambiente.
    \item \textbf{Redes de sensores sem fio (WSN):} muitos nós de baixo consumo, distribuídos em uma área, que medem grandezas físicas (temperatura, umidade, vibração) e as enviam a um ponto de coleta.
    \item Em áreas sem infraestrutura de rede fixa, levar esses dados até o ponto de coleta é um desafio.
  \end{itemize}
  \vspace{0.5em}
  \centering
  \begin{tikzpicture}
...
\begin{frame}{Aplicações no mundo real}
  VANTs já coletam dados de sensores onde a infraestrutura fixa é limitada ou inexistente:
  \begin{itemize}
    \item \textbf{Agricultura de precisão:} dados de sensores de solo e monitoramento de lavouras (DJI Agras, American Robotics).
    ...
    \item \textbf{Resposta a desastres:} coleta contornando a infraestrutura danificada (ex.: sistema CLARKE, Texas A\&M).
